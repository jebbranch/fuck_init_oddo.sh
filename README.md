...
4. 重启设备，或手动执行
```
sh /data/adb/modules/oddo_killer/service.sh
```
测试
...
A: 检查端口是否被占用，或使用
```busybox httpd 是否可用。可尝试手动运行 busybox httpd -p 7891 -h /data/adb/modules/oddo_killer/web``` 测试。（如果端口被占用，请更换文件头部的端口后重新运行脚本）

Q: 运行时计数不准确？

A: 其实计数器没多大作用，能检测到进程然后杀了才是硬道理。

Q: 如何查看调试信息？

A: 在 service.sh 中取消注释 DEBUG_LOG 相关行，然后重启设备/脚本，查看
```
/data/adb/modules/oddo_killer/cache/debug.log。
```
## 贡献
